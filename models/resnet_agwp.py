...
class Network(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.generalize_pool = AGWP(in_channels=2048, emb_dim=opt.embed_dim, num_classes=512)
    ...
    def forward(self, x, **kwargs):
        ...
        for layerblock in self.layer_blocks:
            x = layerblock(x)
        no_avg_feat = x
        _, x, _ = self.generalize_pool(x)  # bs, 2048, 7,7
        enc_out = x = x.view(x.size(0), -1)  # bs, 2048
