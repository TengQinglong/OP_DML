...
class Criterion(nn.Module):
    ...
    def forward(self, batch, labels, **kwargs):
        ...
        similarity = torch.zeros((batch.shape[0], batch.shape[0]), dtype=torch.float)
        for i in range(batch.shape[0]):
            for j in range(i, batch.shape[1]):
                similarity[j][i] = similarity[i][j] = torch.dot(batch[i][j], batch[j][i])/(torch.norm(batch[i][j])*torch.norm(batch[j][i]))
